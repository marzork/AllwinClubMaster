...
##############################################################################################
	...
	def check_win_v3(self, id_number):
		while True:
			...
			for ids in  result['msg']['closed_options']:
				if ids['id'] != None and ids['id'] != [] and ids['id'] != ['']:
					for id_numb in ids['id']:
						if id_number == id_numb:
							return ((ids['win_amount'] - ids['amount'] if ids['win'] != 'equal' else 0),ids["expired"]) 
			time.sleep(1)
	def check_win_v46(self,exp,now,price, ACTIVES, ACTION, expirations):
		try:
			result = self.get_optioninfo_v2(10)
			for ids in  result['msg']['open_options']:
				hora = datetime.strptime(datetime.utcfromtimestamp(int(ids['expired'])).strftime('%H:%M:%S'), '%H:%M:%S')
				expired = str(hora - timedelta(hours=3))
				hora = datetime.strptime(datetime.utcfromtimestamp(int(ids['created'])).strftime('%H:%M:%S'), '%H:%M:%S')
				created = str(hora - timedelta(hours=3))
				hora = datetime.strptime(datetime.utcfromtimestamp(int(exp)).strftime('%H:%M:%S'), '%H:%M:%S')
				exp = str(hora - timedelta(hours=3))
				if  expired[11:-3] == exp[11:-3] and created[11:-3] == str(now)[11:-10]:
					time = int(expired[14:-3]) - int(created[14:-3])
					time2 = int(exp[14:-3]) - int(str(now)[14:-10])
					if price == ids['profit_amount'] and  ACTIVES == ids['active'] and  ACTION == ids['dir']:
						if expirations > 4 and time2 > 3 and time > 3:
							return True, ids['id']
						elif expirations == 1 and time2 <= 2 and time <= 2:
							return True, ids['id']
						else:
							return False, None
			for ids in  result['msg']['closed_options']:
				...
				if  expired[11:-3] == exp[11:-3] and created[11:-3] == str(now)[11:-10]:
					...
					if price == ids['profit_amount'] and  ACTIVES == ids['active'] and  ACTION == ids['dir']:
						if expirations > 4 and time2 > 3 and time > 3:
							return True, ids['id']
						elif expirations > 4 and time2 <= 2 and time <= 2:
							return True, ids['id']
						else:
							return False, None
			return False,None
	...
	def buy(self, price, ACTIVES, ACTION, expirations):
		...
		while self.api.result == None or id == None:
			try:
				if "message" in self.api.buy_multi_option[req_id].keys():
					return False, self.api.buy_multi_option[req_id]["message"],excper,now
			except:
				pass
			try:
				id = self.api.buy_multi_option[req_id]["id"]
			...
			if time.time() - start_t >= 5:
				logging.error('**warning** buy late 5 sec')
				return False, None, excper,now

		return self.api.result, self.api.buy_multi_option[req_id]["id"], excper,now
		...
		if order_data != None:
			if order_data["status"] == "closed":
				if order_data["close_reason"] == "expired":
					return True,order_data["close_profit"]- order_data["invest"],order_data["close_time"]
				elif order_data["close_reason"] == "default":
					return True, order_data["pnl_realized"],order_data["close_time"]
			else:
				return False, None,""
		else:
			return False, None,""
